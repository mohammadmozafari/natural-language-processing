...
# Handling 33 هزار
PATTERN_BEFORE_EXTEND = join_patterns([rf'{PATTERN_NUMBER_WITH_DIGITS}\s*', rf'{PATTERN_SIMPLE_NUMBER}\s+'])
PATTERN_EXTEND = join_patterns(PERSIAN_REQUIRED_EXTENDABLE_NUMBERS)
PATTERN_EXTENDABLE_NUMBER = rf'{PATTERN_BEFORE_EXTEND}{PATTERN_EXTEND}(?:\s+{PATTERN_EXTEND})*'
...
PATTERN_MULTIPLE_NUMBER = rf'{PATTERN_SINGLE_NUMBER}(?:\s+{PERSIAN_V}\s+{PATTERN_SINGLE_NUMBER})*'

ALL_NEGS = join_patterns([rf'{PERSIAN_NEG}\s+', rf'{DIGIT_NEG}\s*'])
